...
import { uploadImage, deleteImage, ImageUploadProgress, ImageUploadResult } from '@/src/services/imageUpload';
...
export async function getMuscleGroupsWithWeeklyStats(startDate: string, endDate: string) {
  ...
  const stats: WeekStat[] = localGroups.map((group) => {
    const weeklySets = workoutLogs.filter((log) => log.muscle_group_id === group.id).length;
...
export async function getMuscleGroupsByWeek(weekStart: string, weekEnd: string) {
  ...
  return localGroups.map((group) => ({
    ...group,
    weeklySets: workoutLogs.filter((log) => log.muscle_group_id === group.id).length,
  }));
}
...
export async function getActiveExercises(muscleGroupId?: string) {
  const exercises = await LocalDB.getExercises(muscleGroupId);
